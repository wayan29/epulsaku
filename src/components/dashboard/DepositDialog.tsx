...
const baseDepositFormSchema = z.object({
  ...
  // Ensure BankEnum here matches the one defined and exported from the flow.
  // The flow exports `BankEnum` as type ["BCA", "MANDIRI", "BRI", "BNI"], so z.enum should use that.
...
interface DigiflazzDepositDialogProps { // Renamed interface
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onDepositSuccess?: () => void; // Callback to refresh balance or similar
}

export default function DigiflazzDepositDialog({ open, onOpenChange, onDepositSuccess }: DigiflazzDepositDialogProps) { // Renamed component
  ...
  const form = useForm<DepositFormValues>({
    ...
    defaultValues: {
      amount: undefined, // Set to undefined for numeric input placeholder
      bank: undefined,
      ownerName: "",
      pin: "",
    },
  ...
  const onSubmit = async (values: DepositFormValues) => {
    ...
    try {
      ...
      if (result.isSuccess) {
        ...
        onDepositSuccess?.(); // Optionally trigger a balance refresh
        // Do not close dialog immediately, show instructions
      } else {
        toast({
