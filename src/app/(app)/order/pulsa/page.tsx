...
export default function PulsaOrderPage() {
  ...
  useEffect(() => {
    const currentRawValue = form.getValues('phoneNumber');
    if (typeof currentRawValue !== 'string') return;

    let digitsOnly = currentRawValue.replace(/\D/g, '');
    let finalNormalizedNumber = digitsOnly;

    if (digitsOnly.startsWith('62')) {
      finalNormalizedNumber = '0' + digitsOnly.substring(2);
    }

    if (finalNormalizedNumber !== currentRawValue) {
      form.setValue('phoneNumber', finalNormalizedNumber, {
        shouldValidate: currentRawValue.length >= 3,
        shouldDirty: true
      });
    }
  }, [watchedPhoneNumber, form]);
  ...
  const handlePinConfirm = async () => {
    ...
    try {
      ...
      await addTransactionToDB(failedTxInput);
      const notificationDetails: TelegramNotificationDetails = {
        refId: refId,
        productName: selectedProduct.product_name,
        customerNoDisplay: `${phoneNumber} (${detectedOperator?.name || selectedProduct.brand})`,
