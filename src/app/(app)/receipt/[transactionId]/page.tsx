...
export default function ReceiptPage() {
  ...
  const handleShare = async () => {
    ...
    const shareData = {
      ...
      text: `My ePulsaku receipt for ${transaction.productName} (Rp ${transaction.sellingPrice.toLocaleString()}). Transaction ID: ${transaction.id.slice(-8)}. SN: ${transaction.serialNumber || 'N/A'}`,
        ...
        <Card className="shadow-lg border-primary/30">
          ...
          <CardContent className="p-6 space-y-4 card-content">
            ...
            <div className="space-y-2 text-sm">
              ...
              {transaction.productCategoryFromDigiflazz && (
                  <div className="flex justify-between text-xs pt-1 details-item">
                      <span className="text-muted-foreground">Category:</span>
                      <span>{transaction.productCategoryFromDigiflazz}</span>
                  </div>
              )}
              {transaction.productBrandFromDigiflazz && (
                  <div className="flex justify-between text-xs details-item">
                      <span className="text-muted-foreground">Brand:</span>
                      <span>{transaction.productBrandFromDigiflazz}</span>
                  </div>
              )}
               <div className="flex justify-between pt-2 details-item">
                <span className="text-muted-foreground">Total Payment:</span>
                <span className="font-bold text-lg text-primary">Rp {transaction.sellingPrice.toLocaleString()}</span>
              </div>
              {/* 
              <div className="flex justify-between text-xs details-item">
                <span className="text-muted-foreground">Harga Modal:</span>
                <span>Rp {transaction.costPrice.toLocaleString()}</span>
              </div>
              <div className="flex justify-between text-xs details-item">
                <span className="text-muted-foreground">Profit:</span>
                <span className="font-semibold text-green-700">Rp {profit.toLocaleString()}</span>
              </div>
              */}
